Project 1: List-Based Priority Queues (Key, Value)

Objective: Implement a generic Key-Value Priority Queue using Java's LinkedList for both unsorted and sorted list approaches.

Requirements:

Create an inner/separate MyEntry<K, V> class to hold the Key and Value.
Both UnsortedListPQ and SortedListPQ must use a java.util.LinkedList internally.
UnsortedListPQ: insert(K, V) is O(1); removeMin() requires traversing the list (O(n)).
SortedListPQ: insert(K, V) requires traversing the list (O(n)); removeMin() is a simple removal from the head (O(1)).
Code Template: Copy and paste the code below into your IDE. You must complete the removeMin method for the unsorted list and the insert method for the sorted list.
